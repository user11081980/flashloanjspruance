...
contract FlashLoanArbitrage2 is FlashLoanSimpleReceiverBase {
    address payable owner;
    address constant VELORA_ROUTER_ADDRESS = 0x6A000F20005980200259B80c5102003040001068;
    ...
    function executeOperation(
        ...
        assetToken.approve(VELORA_ROUTER_ADDRESS, amount);
        (bool success, bytes memory response) = VELORA_ROUTER_ADDRESS.call(data1); // Velora gets the funds from this contract, swaps the amounts, and puts the funds back in this contract (provided that the receiver parameter of the swap API request was left blank).
        if (!success) {
            revert(string.concat("Swap 1 failed.", getRevertReason(response)));
        }

        // Execute the second swap to end up with the borrowed token.
        bridgeToken.approve(VELORA_ROUTER_ADDRESS, bridgeToken.balanceOf(address(this)));
        (success, response) = VELORA_ROUTER_ADDRESS.call(data2);
        if (!success) {
